...
gs.each(function(parent) {
  ...
  let cy =
    startCy -
    calculateRadius(bodyMetrics[0].key, parent[bodyMetrics[0].key]) -
    r;
  const headg = d3
    .select(this)
    .append('g')
  ...
  headg.append('svg:title').text(parent.name);
...
gs.each(function(parent) {
  let cx =
    startCx - calculateRadius(bodyMetrics[0].key, parent[bodyMetrics[0].key]);
  let cy = startCy;
...
gs.each(function(parent) {
  ...
  const lastBodyNode = d3
    .select(`#${parent.name.replace(/\s/g, '')}`)
    .select(`.${lastBodyMetric}`)
    .node();
  let cx =
    Number(lastBodyNode.getAttribute('cx')) -
    cos45 * Number(lastBodyNode.getAttribute('r'));
  let cy =
    Number(lastBodyNode.getAttribute('cy')) +
    cos45 * Number(lastBodyNode.getAttribute('r'));
